...
class LSTMClassifier(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, num_labels, batch_size):
        ...
        self.lstm = nn.LSTM(input_size=embedding_dim, hidden_size=hidden_dim, bidirectional=True) #TODO aca le podemos meter layers, dropout, batch_first
    ...
    def init_hidden(self):
        ...
        return (autograd.Variable(torch.zeros(2, self.batch_size, self.hidden_dim)),
                autograd.Variable(torch.zeros(2, self.batch_size, self.hidden_dim))) # La primera dimensión es 2 porque es bidireccional
